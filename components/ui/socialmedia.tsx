...
export function SocialLink({
    ...
    return (
        <Link className="group -m-1 p-1 flex gap-4" {...props}>
            <Icon className="h-6 w-6 fill-zinc-500 transition group-hover:fill-zinc-600 dark:fill-zinc-400 dark:group-hover:fill-zinc-300" />
            {isLabelNeeded && <span className="font-bold dark:text-white">{label}</span>}
        </Link>
    )
}
...
export default function Socialmedia({isLabelNeeded, style} : {isLabelNeeded: boolean,style?: string}) {
    return (
        <div className={`flex gap-4 ${style}`}>
            ...
            <SocialLink href="https://www.linkedin.com/in/moussa-dieng-aa69a1226/" isLabelNeeded={isLabelNeeded} aria-label="Follow on LinkedIn" label="Follow on LinkedIn" icon={LinkedInIcon}/>
        </div>
    )
}